...
__date__ = u'2024/12/10 10:37'
__software__ = u'PyCharm'
__email__ = u'zhouyajun@yikongenomics.com'
__author__ = u'\u5468\u4e9a\u519b'
__describe__ = u'QC_result_ReportJson4Marsala_InstPGTA_v1.2.py'
...
#  5) 2023年12月8日 -- 根据报告端反馈适当调整QC_report.JSON对接输入文件,兼容10月后的输出文件版本，L759 & L782
...
def Run_CNV_report(platform_type, CNV_Path, project_name, ZIP_res_dir, temp_roh):
    # PGD & IB_PGD
...
def main():
    ...
    if "_analysis/result" in input_results_:
        ...
        # 分析目录或者结果目录都可以
        input_results = F"{Path(input_results_).as_posix()}/pgta_analysis/result"
    ...
    if CNV_zip:
        ...
        if not temp_local:
            logger.error(F"没有检查到CNV/local_report目录,退出!")
            sys.exit(1)
    ...
    #    sys.exit(0)
    # elif re.findall("PGTA|MaReCs|PGT", project_name):
    if re.findall("PGTA|MaReCs|PGT", project_name):
        logger.info(f"识别到项目：{project_name}")
        # TODO CNV 部分功能输出,并且输出JSON后解压压缩包到CNV目录
        # 2) 调整后的CNV路径进行直接读取整合
        Run_CNV_report(platform_type, CNV_Path, project_name, ZIP_res_dir, temp_roh)
        ...
        if Path(QC_file).exists():
            ...
            # 3) SNP部分逻辑需要追加QC部分
            PGTA_Type(ZIP_res_dir).DoSomethingPGTA()
...
class PGTA_Type(object):
    ...
    def Get_snp2cnv(self):
        ...
        # info_dd = df.to_dict("index")
        new_dd = {}
        for k, v in info_dd.items():
            new_dd[k] = v['Heteroploid_result']
        return new_dd, info_dd
    
    def DoSomethingPGTA(self, res_file="QC_Report"):
        dd = defaultdict(dict)
        all_sample = []
        uneven_dir1 = F"{self.ZIP_res_dir}/uneven"
        uneven_dir2 = F"{self.ZIP_res_dir}/vaf"
        if Path(uneven_dir1).exists():
            uneven_dir = uneven_dir1
        elif Path(uneven_dir2).exists():
            uneven_dir = uneven_dir2
        else:
            logger.error(F"找不到分析目录的uneven/vaf 目录")
            sys.exit(1)
        ...
        logger.debug(F"全部样本：{all_sample}")
        result = {}
        for key in all_sample:
            ...
            # sample_dd.update({"ASA_QC_Data": t2})
            # print(sample_dd, {"ASA_QC_Data": t2})
