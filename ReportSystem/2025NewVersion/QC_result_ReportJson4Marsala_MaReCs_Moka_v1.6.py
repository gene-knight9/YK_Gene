...
def Copy_moka_result(analysis_dir):
    ...
    if  Path(file_).exists():
        ...
        if  all([Path(report_dir).exists(), Path(result_cmoka_).exists()]):
            ...
            hic_file = F"{report_dir}/SV/*.hic"
            hic_list = glob(hic_file)
            if hic_list:
                hic_str = " ".join(hic_list)
                cmd_hic = f"cp  {hic_str}  {result_cmoka_}"
                logger.debug(f"copying hic file -> {cmd_hic}")
                ExecuteCMD(cmd_hic)
            else:
                logger.error("不存在hic文件")
            version_file = f"{report_dir}/version.json"
            if Path(version_file).exists():
                logger.debug(f"copy {version_file=}")
                shutil.copy(version_file, result_cmoka_)
            else:
                logger.error("c-moka版本文件不存在")
...
def main():
    ...
    Copy_moka_result(input_results_)
    # /data/Project_2023Q3/Project_YKSZ_MaReCs2_230912_02E_01_1035
