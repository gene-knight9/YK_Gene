...
__date__ = u'2025/09/04 17:37'
__software__ = u'PyCharm'
__email__ = u'zhouyajun@yikongenomics.com'
__author__ = u'\u5468\u4e9a\u519b'
__describe__ = u'QC_result_ReportJson4Marsala_MaReCs_Moka_v1.8.py'
...
#  7) 2025年8月7日 -- moka结果由marecs_moka_results.tsvh换为final_marecsMoKa_results.tsv
#  8) 2025年8月28日 -- mokaE强行拷贝verson.json和.hic
#  9) 2025年9月4日 -- 读取配置文件内的report目录，强制重新拷贝所有的.png和.hic,version.json， 并且新增目录mokaB，外层目录统一挪进去打包。
# 10) 2025年9月10 -- 增加"CNV检测结果（来源）"到_result_Report.json中
...
def arguments():
    ...
    parser.add_argument('--ZipType', '-t', dest='ZT', type=str, default="MaReCs",
                        required=False, choices=["MaReCs1", "MaReCs2", "MaReCs" , "MoKaB"],
...
class CNV_NewPipeReport(object):
    ...
    def Format_结果说明(result_description):
        ...
        if isinstance(result_description, str):
            ...
            for i, v in enumerate(result_description, start=1):
                RD += f"{i}、{v}\n"
        else:
        ...
        if not SNP and CNV:
            ...
            if analysis_type == "CNV":
                ...
                if re.findall("_MaReCs1|_PGTSR|_PGT", project_name):
                    ...
                    with open(save_file2, 'w', encoding="utf-8") as R_CNV:
                        json.dump(CNV_NewPipeReport(ad, TCD, TCDIF, temp_roh, NICS=None, flag_CDR=flag_chimeDetectionResult,orgin_dict=orgin_CNV_info).Add_Breakpoint2SampleDict(),
                                  R_CNV, sort_keys=True, ensure_ascii=False, indent="\t")
...
class Check_Marsala_MaReCs_results(object):
    def __init__(self, input_dir, marecs_type):
        ...
        self.result_dir = F"{self.input_dir}/moka_analysis/result"
    ...
    def Get_All_Kinds_files(self):
        ...
        res_kinds = {"CNV": ["CNV/*.zip"],
                     ...
                     "kinship": ["*.king.kinship.xlsx",
                      ...
                      "MoKaE": ["mokaE/*_SVHiKER.report.txt",
                               "mokaE/*_SVHiKER_[MF].png",
                               "mokaE/moka_report_info.txt", 
                               "mokaE/final_marecsMoKa_results.tsv"],
                      "MoKaB":["*_multiqc_report.html",
                               "*_SVHiKER.report.txt",
                               "*_SVHiKER_[MF].png",
                               "haplotype*filter.docx",
                              "carrier_iSNP_summary.txt"
                      ]}
        parent_res_dir = [i.as_posix() for i in Path(dir_res).glob("*") if i.is_dir()][0]
        check_list = []
        if  self.type_marecs == "MoKaB":
            logger.info("指定检查类型为 MoKaB")
            logger.debug(F"检查文件夹：{parent_res_dir}")
            print(f"{'Check MoKaB files!':=^{terminal_width}}")
            MoKaB_Conut = self.Count_error(parent_res_dir, res_kinds["MoKaB"])
            if MoKaB_Conut > 0:
        ...
        print(f"{'Check MoKaE files!':=^{terminal_width}}")
        MoKaE_conut = self.Count_error(parent_res_dir, res_kinds["MoKaE"])
        ...
        if CNV_count != 0:
            ...
            if ks_count > 0:
                ...
                logger.critical("MoKaE文件缺失， 请核对后重试")
                check_list.append(False)
            else:
                check_list.append(True)
...
def Copy_moka_result(analysis_dir,zip_type):
    name_pro = Path(analysis_dir).name.replace("Project_","")
    file_ = f"{analysis_dir}/config.yaml"
    sample_dir = f"{analysis_dir}/sample_info/ori_json"
    if  Path(file_).exists():
        with open(file_, "r") as ym:
            dict_yaml = yaml.safe_load(ym)
        report_dir = dict_yaml.get("MaReCs_MoKa",{}).get("directories",{}).get("cmoka", {}).get("result_dir", "")
        if zip_type!="MoKaB":
            result_cmoka_ = f"{analysis_dir}/moka_analysis/result/{name_pro}/mokaE"
        else:
            result_cmoka_ =  f"{analysis_dir}/moka_analysis/result/{name_pro}"
        if  all([Path(report_dir).exists(), Path(result_cmoka_).exists()]):
            logger.info(f"checking {report_dir=}  {result_cmoka_=}")
            copy_ohers = f"cp  {report_dir}/version.json  {report_dir}/SV/*png  {report_dir}/SV/*.hic  {result_cmoka_}"
            logger.debug(f"{copy_ohers=}")
            ExecuteCMD(copy_ohers)

    all_pro = []
    if Path(sample_dir).exists():
        for  each in Path(sample_dir).glob("./*json"):
            name_json = each.stem
            all_pro.append(name_json)
        sub_pro = [i for  i in all_pro  if  "C-MoKa" in i]
        if  sub_pro:
            str_comka = "_".join(sub_pro)
            file_txt = f"{result_cmoka_}/关联C-MoKa项目{str_comka}.txt"
            Path(file_txt).touch()
            Path(file_txt).chmod(0o755)
...
def main():
    ...
    Copy_moka_result(input_results_,ZipType)
    global logger
    CNV_zip = []
    if re.findall("moka_analysis/result", input_results_):
        input_results = input_results_
        cnv_run_dir = Path(input_results_).parent / "CNV"
    else:
        # 分析目录或者结果目录都可以
        input_results = F"{Path(input_results_).as_posix()}/moka_analysis/result"
        cnv_run_dir =  Path(input_results_) / "moka_analysis"/ "CNV"
    ...
    if ZipType == "MoKaB":
        logger.info(f"直接压缩")
        result_mokaB = f"{ZIP_res_dir}/mokaB"
        rename_dir(Path(ZIP_res_dir), Path(result_mokaB))
        rename_dir(Path(input_results), Path(ZIP_res_dir))
    ...
    if re.findall("_Control_", project_name, flags=re.IGNORECASE):
        logger.error("该流程Control项目|MoKaB直接不处理,直接zip压包")
        ZIP_result(ZIP_res_dir, zip_file)
        sys.exit(0)
    


    elif re.findall("MoKa", project_name, flags=re.IGNORECASE):
...
def ZIP_result(zip_dir, zip_package_file):
    logger.debug(f"压缩{zip_dir}-->{zip_package_file}")
